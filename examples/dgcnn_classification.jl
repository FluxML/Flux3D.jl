...
function get_processed_data(args)
    ...
    train = [(cat(data[i]..., dims = 3), labels[:,i]) for i in partition(1:length(data), batch_size)] .|> args.device
...
function train(; kwargs...)
    ...
    custom_train!(loss, params(m), train, opt, args.epochs, val)

    return m
end

function custom_train!(loss, ps, data, opt, epochs, (valX, valY))
    ps = Zygote.Params(ps)
